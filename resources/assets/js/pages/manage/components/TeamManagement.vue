                ...
                <ul>
                    ...
                    <li
                        ...
                        <EDropdown
                            ...
                            <i @click.stop="" class="taskfont department-menu">&#xe6e9;</i>
                            <EDropdownMenu slot="dropdown">
                                <EDropdownItem v-if="item.level <= 9" :command="`add_${item.id}`">
                                    <div>{{$L('添加子部门')}}</div>
                                </EDropdownItem>
                ...
                <FormItem prop="parent_id" :label="$L('上级部门')">
                    <Select v-model="departmentData.parent_id" :placeholder="$L('请选择上级部门')">
                        ...
                        <Option
                            ...
                            :disabled="item.level > 9 || item.id == departmentData.id || (item.parent_id == departmentData.id && departmentData.id > 0)"
                            :value="item.id"
                            :key="index"
